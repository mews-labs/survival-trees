...
def auc_vs_score_plotly(
    ...
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                        vertical_spacing=0.08, row_heights=[1000, 550])
    t = True
    for d in np.unique(data.index):
        data_t = data.loc[d]
        u = + np.random.rand() * 0.05
        color = (event_observed_color
                 if data_t.loc[d]["observed event"].iloc[0]
                 else event_censored_color)
        ...
        fig.add_traces(
            [go.Scatter(
                ...
                mode='lines',
                name="Censored event",
                line={"color": event_censored_color, "width": 1},
    ...
    for i in range(len(temporal_metric) - 1):
        tmp_legend = i <= 0
        fig.add_traces(
            [go.Scatter(x=[temporal_metric.index[i],
                           temporal_metric.index[i + 1]],
                        y=[temporal_metric.iloc[i],
                           temporal_metric.iloc[i + 1]],
                        showlegend=tmp_legend, name="Time dependent AUC",
    ...
    fig.add_traces(
        [go.Scatter(x=[temporal_metric.index.tolist()[-1]],
                    y=[temporal_metric.values[-1]],
                    mode="markers",
    ...
    if plot_random_prediction:
        fig.add_traces(
            [go.Scatter(x=[temporal_metric.index[0],
                           temporal_metric.index[-1]],
                        y=[0.5, 0.5],
